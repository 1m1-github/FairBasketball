...
This has led to a concentration of skills in the league, such as either being a 3-point expert or a paint (close to basket) expert. Essentially, most shots are attempted from the 3-point-line. Making the game less varied makes it less interesting.

The purpose of this paper is to demonstrate new scoring rules that make every shot potentially equally efficient.

== Probabilities

Given any spot on the court, there is a probability $p$ of making the shot from there, which decreases as the distance to the hoop $d$ increases and decreases as the angle to the backboard $alpha$ increases.
...
We want $E ident E(d, alpha)$ to be constant across the court. We can arbitrarily set as $E ident 1$
...
E.g. for a layup, we get $p(0, 0) approx 100% arrow.r.double S(0, 0) = 1$, i.e. a layup gives 1 point.
_If_ a half-court shot has probability 2% to be made, it would get 50 points.
